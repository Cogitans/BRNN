...
class ScaledGRU(GRU):
    ...
    def step(self, x, states):
        ...
        x_z = tf.multiply(tf.pow(2.0, self.B), K.dot(x, self.W_z)) + self.b_z
        x_r = tf.multiply(tf.pow(2.0, self.B), K.dot(x, self.W_r)) + self.b_r
        x_h = tf.multiply(tf.pow(2.0, self.B), K.dot(x, self.W_h)) + self.b_h
        ...
        hh = self.activation(x_h + tf.multiply(tf.pow(2.0, self.A), K.dot(r * h_tm1, self.U_h)))
        h = z * h_tm1 + (1 - z) * hh
        return h, [h]
